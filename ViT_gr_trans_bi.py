...
def init_dataset():
    print("Load datasets...")
    train_dataset = semgDataset("preprocessed/new_split/idx_list_train_paper.csv", "preprocessed/new_split/train_labels.csv", classification_type='bio_id')
    test_dataset = semgDataset("preprocessed/new_split/idx_list_test_paper.csv", "preprocessed/new_split/test_labels.csv", classification_type='bio_id')
    val_dataset = semgDataset("preprocessed/new_split/idx_list_val_paper.csv", "preprocessed/new_split/val_labels.csv", classification_type='bio_id')
...
def train(tr_dataloader, model, optim, scheduler, val_dataloader=None):
    ...
    torch.save(model.state_dict(), 'models/paper_params/ViT_gr_trans_bi.pth')
...
if __name__ == "__main__":
    ...
    model = model.cuda()
    model.load_state_dict(torch.load('models/paper_params/ViT_gr.pth'))
    ...
    model.head = nn.Linear(num_ftrs, 21)
