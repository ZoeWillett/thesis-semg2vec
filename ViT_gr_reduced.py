# importing libraries
import os
from utils_reduced import init_dataset
import torch
...
def init_model():
    model = VisionTransformer(
                       ...
                       in_chans=8, 
                       n_classes=56, # 66 for gest rec / 21 for bio id / 56 for reduced 
                       embed_dim=64,
                       depth=1, 
                       n_heads=8, 
                       mlp_ratio=1, 
...
def train(tr_dataloader, model, optim, scheduler, val_dataloader=None):
    ...
    torch.save(model.state_dict(), 'models/paper_params/ViT_gr_reduced.pth')
